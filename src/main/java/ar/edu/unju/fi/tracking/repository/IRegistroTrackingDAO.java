...
public interface IRegistroTrackingDAO extends JpaRepository<RegistroTracking, Long> {
	...
	@Query(value="select * from registros where id_vehiculo in (select id_vehiculo from vehiculos where patente = ?1)",nativeQuery = true)
	public List<RegistroTracking> listarRegistrosPorPatente(String patente);
	
	@Query(value="select * from registros where id_registro in(select id_registro from registro_tripulante where id_tripulante in (select id_tripulante from tripulantes where documento = ?1))",nativeQuery = true)
